...
import { BellIcon, ClockIcon, CheckCircleIcon, XCircleIcon, EnvelopeIcon, decoded } from '@heroicons/react/24/outline';
          ...
          {Object.entries(settings).map(([channel, enabled]) => (
            <label key={channel} className="flex items-center gap-3 cursor-pointer">
              <input
                type="checkbox"
                checked={enabled}
                onChange={(e) => decoded(channel, e.target.checked)}
                className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:decoded focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
              />
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-start gap-3">
                <div className={`p-2 rounded-lg ${decoded(reminder.type)}`}>
